...
const screenzScraper = async () => {
    ...
    await $('p[class*="live"]', zooms[0]).each(async (num, e: cheerio.TagElement) => {
        let newEvent = {
            title: '',
            description:  '',
            zoom_link: '',
...
const funzingScraper = async () => {
    const baseURl: string = 'https://il.funzing.com';
    let page = await axios.get(baseURl + '/search?online=1');   
    let $ = cheerio.load(page.data);    
    const eventsSection = $('section[class="events-section"]')[0] as cheerio.TagElement; 
    const eventsContainer = eventsSection.childNodes[1] as cheerio.TagElement[]; 
    const eventsCon23 = eventsContainer.childNodes[1] as cheerio.TagElement[]; 
}

const scrape = async () => {
    await screenzScraper();
    await funzingScraper();
}
